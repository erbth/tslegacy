...
declare -a PKG_DIRS
for DIR in ${PACKAGING_LOCATION}/{libepoxy,libepoxy-dev}/${DESTDIR}
do
    PKG_DIRS+=($DIR)
    rm -rf ${DIR}/*
done
...
if [ -d "${PKG_DIRS[1]}/usr/share/doc/libepoxy" ]
then
    mv "${PKG_DIRS[1]}/usr/share/doc/libepoxy"{,-dev}
fi

install_readme_files "${PKG_DIRS[1]}" libepoxy-dev

# libepoxy
mv ${INSTALL_DIR}/target/* ${PKG_DIRS[0]}/
install_readme_files "${PKG_DIRS[0]}" libepoxy
