...
function install_readme_files
{
    ...
    for FILE in "${BUILD_DIR}/${SRC_DIR}"/{CREDITS*,CODE_OWNERS*,README*,readme*,COPYING*,Copyright*,LICENSE*,license*,NOTICE,NOTES,THANKS,AUTHORS} \
        "${BUILD_DIR}/${SRC_DIR}/doc"/{faq.html,license.html,patents.txt,README*,relnotes.html,thanks.html,VERSIONS}
    do
        if [ -r "$FILE" ]
        then
...
declare -a PKG_DIRS
for DIR in ${PACKAGING_LOCATION}/{mesa,mesa-dev}/${DESTDIR}
do
    PKG_DIRS+=($DIR)
    rm -rf ${DIR}/*
done
...
if [ -d "${PKG_DIRS[1]}/usr/share/doc/mesa" ]
then
    mv "${PKG_DIRS[1]}/usr/share/doc/mesa"{,-dev}
fi

install_readme_files "${PKG_DIRS[1]}" mesa-dev

# mesa
mv ${INSTALL_DIR}/target/* ${PKG_DIRS[0]}/
install_readme_files "${PKG_DIRS[0]}" mesa
