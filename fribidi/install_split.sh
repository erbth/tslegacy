...
declare -a PKG_DIRS
for DIR in ${PACKAGING_LOCATION}/{fribidi,fribidi-dev,fribidi-libs}/${DESTDIR}
do
    PKG_DIRS+=($DIR)
    rm -rf ${DIR}/*
done
...
if [ -d "${PKG_DIRS[1]}/usr/share/doc/fribidi" ]
then
    mv "${PKG_DIRS[1]}/usr/share/doc/fribidi"{,-dev}
fi

install_readme_files "${PKG_DIRS[1]}" fribidi-dev
...
install_readme_files "${PKG_DIRS[2]}" fribidi-libs

# fribidi
mv ${INSTALL_DIR}/target/* ${PKG_DIRS[0]}/
install_readme_files "${PKG_DIRS[0]}" fribidi
