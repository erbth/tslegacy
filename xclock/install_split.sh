...
declare -a PKG_DIRS
for DIR in ${PACKAGING_LOCATION}/{xclock,xclock-dev}/${DESTDIR}
do
    PKG_DIRS+=($DIR)
    rm -rf ${DIR}/*
done
...
if [ -d "${PKG_DIRS[1]}/usr/share/doc/xclock" ]
then
    mv "${PKG_DIRS[1]}/usr/share/doc/xclock"{,-dev}
fi

install_readme_files "${PKG_DIRS[1]}" xclock-dev

# xclock
mv ${INSTALL_DIR}/target/* ${PKG_DIRS[0]}/
install_readme_files "${PKG_DIRS[0]}" xclock
