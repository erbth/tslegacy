...
declare -a PKG_DIRS
for DIR in ${PACKAGING_LOCATION}/{xkeyboard-config,xkeyboard-config-dev}/${DESTDIR}
do
    PKG_DIRS+=($DIR)
    rm -rf ${DIR}/*
done
...
if [ -d "${PKG_DIRS[1]}/usr/share/doc/xkeyboard-config" ]
then
    mv "${PKG_DIRS[1]}/usr/share/doc/xkeyboard-config"{,-dev}
fi

install_readme_files "${PKG_DIRS[1]}" xkeyboard-config-dev

# xkeyboard-config
mv ${INSTALL_DIR}/target/* ${PKG_DIRS[0]}/
install_readme_files "${PKG_DIRS[0]}" xkeyboard-config
