...
declare -a PKG_DIRS
for DIR in ${PACKAGING_LOCATION}/{libdrm,libdrm-dev}/${DESTDIR}
do
    PKG_DIRS+=($DIR)
    rm -rf ${DIR}/*
done
...
if [ -d "${PKG_DIRS[1]}/usr/share/doc/libdrm" ]
then
    mv "${PKG_DIRS[1]}/usr/share/doc/libdrm"{,-dev}
fi

install_readme_files "${PKG_DIRS[1]}" libdrm-dev

# libdrm
mv ${INSTALL_DIR}/target/* ${PKG_DIRS[0]}/
install_readme_files "${PKG_DIRS[0]}" libdrm
