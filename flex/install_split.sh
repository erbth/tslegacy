...
declare -a PKG_DIRS
for DIR in ${PACKAGING_LOCATION}/{flex,flex-dev,flex-libs}/${DESTDIR}
do
    PKG_DIRS+=($DIR)
    rm -rf ${DIR}/*
done
...
if [ -d "${PKG_DIRS[1]}/usr/share/doc/flex" ]
then
    mv "${PKG_DIRS[1]}/usr/share/doc/flex"{,-dev}
fi

install_readme_files "${PKG_DIRS[1]}" flex-dev
...
install_readme_files "${PKG_DIRS[2]}" flex-libs

# flex
mv ${INSTALL_DIR}/target/* ${PKG_DIRS[0]}/
install_readme_files "${PKG_DIRS[0]}" flex
