...
declare -a PKG_DIRS
for DIR in ${PACKAGING_LOCATION}/{rxvt-unicode,rxvt-unicode-dev}/${DESTDIR}
do
    PKG_DIRS+=($DIR)
    rm -rf ${DIR}/*
done
...
if [ -d "${PKG_DIRS[1]}/usr/share/doc/rxvt-unicode" ]
then
    mv "${PKG_DIRS[1]}/usr/share/doc/rxvt-unicode"{,-dev}
fi

install_readme_files "${PKG_DIRS[1]}" rxvt-unicode-dev

# rxvt-unicode
mv ${INSTALL_DIR}/target/* ${PKG_DIRS[0]}/
install_readme_files "${PKG_DIRS[0]}" rxvt-unicode
