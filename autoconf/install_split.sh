...
{
    for FILE in "${BUILD_DIR}/${SRC_DIR}"/{README,COPYING.EXCEPTION,NOTICE,NOTES,THANKS,AUTHORS}
    do
        if [ -r "$FILE" ]
        then
...
declare -a PKG_DIRS
for DIR in ${PACKAGING_LOCATION}/{autoconf,autoconf-dev}/${DESTDIR}
do
    PKG_DIRS+=($DIR)
    rm -rf ${DIR}/*
done
...
if [ -d "${PKG_DIRS[1]}/usr/share/doc/autoconf" ]
then
    mv "${PKG_DIRS[1]}/usr/share/doc/autoconf"{,-dev}
fi

install_readme_files "${PKG_DIRS[1]}" autoconf-dev

# autoconf
mv ${INSTALL_DIR}/target/* ${PKG_DIRS[0]}/
install_readme_files "${PKG_DIRS[0]}" autoconf
