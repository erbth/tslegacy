...
then
    ...
    chroot ${TPM_TARGET} /usr/bin/localedef --prefix=${TPM_TARGET} -i en_US -f ISO-8859-1 en_US
    chroot ${TPM_TARGET} /usr/bin/localedef --prefix=${TPM_TARGET} -i en_US -f UTF-8 en_US.UTF-8
fi
