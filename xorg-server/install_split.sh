...
declare -a PKG_DIRS
for DIR in ${PACKAGING_LOCATION}/{xorg-server,xorg-server-dev}/${DESTDIR}
do
    PKG_DIRS+=($DIR)
    rm -rf ${DIR}/*
done
...
if [ -d "${PKG_DIRS[1]}/usr/share/doc/xorg-server" ]
then
    mv "${PKG_DIRS[1]}/usr/share/doc/xorg-server"{,-dev}
fi

install_readme_files "${PKG_DIRS[1]}" xorg-server-dev

# xorg-server
mv ${INSTALL_DIR}/target/* ${PKG_DIRS[0]}/
install_readme_files "${PKG_DIRS[0]}" xorg-server
