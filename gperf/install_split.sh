...
{
    for FILE in "${BUILD_DIR}/${SRC_DIR}"/{README,NOTICE,NOTES}
    do
        if [ -r "$FILE" ]
        then
...
declare -a PKG_DIRS
for DIR in ${PACKAGING_LOCATION}/{gperf,gperf-dev}/${DESTDIR}
do
    PKG_DIRS+=($DIR)
    rm -rf ${DIR}/*
done
...
if [ -d "${PKG_DIRS[1]}/usr/share/doc/gperf" ]
then
    mv "${PKG_DIRS[1]}/usr/share/doc/gperf"{,-dev}
fi

install_readme_files "${PKG_DIRS[1]}" gperf-dev

# gperf
mv ${INSTALL_DIR}/target/* ${PKG_DIRS[0]}/
install_readme_files "${PKG_DIRS[0]}" gperf
