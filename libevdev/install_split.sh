...
declare -a PKG_DIRS
for DIR in ${PACKAGING_LOCATION}/{libevdev,libevdev-dev,libevdev-libs}/${DESTDIR}
do
    PKG_DIRS+=($DIR)
    rm -rf ${DIR}/*
done
...
if [ -d "${PKG_DIRS[1]}/usr/share/doc/libevdev" ]
then
    mv "${PKG_DIRS[1]}/usr/share/doc/libevdev"{,-dev}
fi

install_readme_files "${PKG_DIRS[1]}" libevdev-dev
...
install_readme_files "${PKG_DIRS[2]}" libevdev-libs

# libevdev
mv ${INSTALL_DIR}/target/* ${PKG_DIRS[0]}/
install_readme_files "${PKG_DIRS[0]}" libevdev
