...
declare -a PKG_DIRS
for DIR in ${PACKAGING_LOCATION}/{automake,automake-dev}/${DESTDIR}
do
    PKG_DIRS+=($DIR)
    rm -rf ${DIR}/*
done
...
if [ -d "${PKG_DIRS[1]}/usr/share/doc/automake" ]
then
    mv "${PKG_DIRS[1]}/usr/share/doc/automake"{,-dev}
fi

install_readme_files "${PKG_DIRS[1]}" automake-dev

# automake
mv ${INSTALL_DIR}/target/* ${PKG_DIRS[0]}/
install_readme_files "${PKG_DIRS[0]}" automake
