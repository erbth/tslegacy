...
declare -a PKG_DIRS
for DIR in ${PACKAGING_LOCATION}/{font-util,font-util-dev}/${DESTDIR}
do
    PKG_DIRS+=($DIR)
    rm -rf ${DIR}/*
done
...
if [ -d "${PKG_DIRS[1]}/usr/share/doc/font-util" ]
then
    mv "${PKG_DIRS[1]}/usr/share/doc/font-util"{,-dev}
fi

install_readme_files "${PKG_DIRS[1]}" font-util-dev

# font-util
mv ${INSTALL_DIR}/target/* ${PKG_DIRS[0]}/
install_readme_files "${PKG_DIRS[0]}" font-util
