...
declare -a PKG_DIRS
for DIR in ${PACKAGING_LOCATION}/{xml-parser,xml-parser-dev}/${DESTDIR}
do
    PKG_DIRS+=($DIR)
    rm -rf ${DIR}/*
done
...
if [ -d "${PKG_DIRS[1]}/usr/share/doc/xml-parser" ]
then
    mv "${PKG_DIRS[1]}/usr/share/doc/xml-parser"{,-dev}
fi

install_readme_files "${PKG_DIRS[1]}" xml-parser-dev

# xml-parser
mv ${INSTALL_DIR}/target/* ${PKG_DIRS[0]}/
install_readme_files "${PKG_DIRS[0]}" xml-parser
