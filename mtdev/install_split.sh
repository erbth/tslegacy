...
declare -a PKG_DIRS
for DIR in ${PACKAGING_LOCATION}/{mtdev,mtdev-dev,mtdev-libs}/${DESTDIR}
do
    PKG_DIRS+=($DIR)
    rm -rf ${DIR}/*
done
...
if [ -d "${PKG_DIRS[1]}/usr/share/doc/mtdev" ]
then
    mv "${PKG_DIRS[1]}/usr/share/doc/mtdev"{,-dev}
fi

install_readme_files "${PKG_DIRS[1]}" mtdev-dev
...
install_readme_files "${PKG_DIRS[2]}" mtdev-libs

# mtdev
mv ${INSTALL_DIR}/target/* ${PKG_DIRS[0]}/
install_readme_files "${PKG_DIRS[0]}" mtdev
