...
declare -a PKG_DIRS
for DIR in ${PACKAGING_LOCATION}/{wayland,wayland-dev}/${DESTDIR}
do
    PKG_DIRS+=($DIR)
    rm -rf ${DIR}/*
done
...
if [ -d "${PKG_DIRS[1]}/usr/share/doc/wayland" ]
then
    mv "${PKG_DIRS[1]}/usr/share/doc/wayland"{,-dev}
fi

install_readme_files "${PKG_DIRS[1]}" wayland-dev

# Move a development utility to the dev package
#
# I read about wayland-scanner in the book
...
mv usr/bin/wayland-scanner "${PKG_DIRS[1]}/usr/bin/"

# wayland
mv ${INSTALL_DIR}/target/* ${PKG_DIRS[0]}/
install_readme_files "${PKG_DIRS[0]}" wayland
