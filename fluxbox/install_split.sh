...
declare -a PKG_DIRS
for DIR in ${PACKAGING_LOCATION}/{fluxbox,fluxbox-dev}/${DESTDIR}
do
    PKG_DIRS+=($DIR)
    rm -rf ${DIR}/*
done
...
if [ -d "${PKG_DIRS[1]}/usr/share/doc/fluxbox" ]
then
    mv "${PKG_DIRS[1]}/usr/share/doc/fluxbox"{,-dev}
fi

install_readme_files "${PKG_DIRS[1]}" fluxbox-dev

# fluxbox
mv ${INSTALL_DIR}/target/* ${PKG_DIRS[0]}/
install_readme_files "${PKG_DIRS[0]}" fluxbox
