...
declare -a PKG_DIRS
for DIR in ${PACKAGING_LOCATION}/{pixman,pixman-dev}/${DESTDIR}
do
    PKG_DIRS+=($DIR)
    rm -rf ${DIR}/*
done
...
if [ -d "${PKG_DIRS[1]}/usr/share/doc/pixman" ]
then
    mv "${PKG_DIRS[1]}/usr/share/doc/pixman"{,-dev}
fi

install_readme_files "${PKG_DIRS[1]}" pixman-dev

# pixman
mv ${INSTALL_DIR}/target/* ${PKG_DIRS[0]}/
install_readme_files "${PKG_DIRS[0]}" pixman
